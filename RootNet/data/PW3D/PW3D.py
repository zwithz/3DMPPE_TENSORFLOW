...
from ...common import cam2pixel, pixel2cam, process_bbox
...
class PW3D:
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            joint_valid = ((joint_img[:, 0] >= 0) * (joint_img[:, 0] < img_width) * (joint_img[:, 1] >= 0) * (
    ...
    def evaluate(self, preds, result_dir, epoch):
        ...
        for n in range(sample_num):
            ...
            pred_root_coord = pred_root_coord.reshape(3)
            pred_save.append(
                {'image_id': img_id, 'ann_id': ann_id, 'bbox': bbox.tolist(), 'root_cam': pred_root_coord.tolist()})
        ...
        print('MRPE >> x: ' + str(err_x) + ' y: ' + str(err_y) + ' z: ' + str(err_z) + ' total: ' + str(
            err_total))  # error print (meter)

        output_path = osp.join(result_dir, 'rootnet_pw3d_output_epoch%d.json' % epoch)
