...
import numpy as np
from ...common import process_bbox
...
class MuCo:
    def __init__(self, data_split):
        ...
        self.joint_num = 21
        self.joints_name = ('Head_top', 'Thorax', 'R_Shoulder', 'R_Elbow', 'R_Wrist',
        ...
        for iid in db.imgs.keys():
            ...
            for i in range(len(anns)):
                ...
                for j in range(len(anns)):
                    if i == j:
                        continue
                    dist = (np.array(anns[i]['keypoints_cam'][self.root_idx]) - np.array(
                        anns[j]['keypoints_cam'][self.root_idx])) ** 2
